...
namespace RFIDTrackBin.fragment
{
    public class EntradasFragment : BaseFragment, IReaderEventListener, IRfidUhfEventListener, MainReceiver.IEventLitener
    {
        ...
        private List<string> fpList = new List<string>();
        ...
        public override async void OnViewCreated(View view, Bundle savedInstanceState)
        {
            ...
            //if (!_activity.Privilegios.MostrarEntradas)
            //{
            //    Toast.MakeText(_activity, "Entradas disponibles de 7:00 a.m. a 4:00 a.m.", ToastLength.Long).Show();
            //    _activity.SupportFragmentManager.PopBackStack();
            //    return;
            //}
            #endregion
        ...
        public override bool OnOptionsItemSelected(IMenuItem item)
        {
            switch (item.ItemId)
            {
                case Resource.Id.inicio_entradas:
                    InsertarEntrada(tipoMovimiento, ((MainActivity)Activity).usuario, prov_clave, rch_clave, tbl_clave, "A");
                    _menu?.FindItem(Resource.Id.inicio_entradas)?.SetEnabled(false);
                    _menu?.FindItem(Resource.Id.final_entradas)?.SetEnabled(true);
                    btnGuardar.Enabled = true;
                    // Deshabilitar ítems del BottomNavigationView (excepto Entradas)
                    _activity.DisableNavigationItems(Resource.Id.navigation_inventario, Resource.Id.navigation_salidas, Resource.Id.navigation_entradas);
                    return true;
        ...
        {
            ((AndroidX.AppCompat.App.AppCompatActivity)Activity).SupportActionBar.Title = "ENTRADAS";
            base.OnResume();
        ...
        private void MostrarDialogoFletesPendientes()
        {
    ...
    new Android.App.AlertDialog.Builder(_activity, Resource.Style.AppTheme_CustomAlertDialog);
            ...
            builder.SetPositiveButton("CERRAR", (sender, args) =>
            {
                ...
                fletesPendientesDialog?.Dismiss(); // Usa Dismiss para cerrar el diálogo
                fletesPendientesDialog = null;
                //getTb_RFID_Catalogo();
            });
            fletesPendientesDialog = builder.Create();
            fletesPendientesDialog.SetTitle("Fletes Pendientes");

            fletesPendientesDialog.Show();
        }

        private void getFletesPendientes()
        {
            fpList.Clear();

            string query = @"SELECT A.id_flete, A.nom_proveedor, B.orde, A.id_destino 
                     FROM tb_mstr_flete A
                     JOIN tb_cat_origen_destino B ON A.origen = B.id_od
                     WHERE A.estatus = 'A' AND B.orde LIKE '%COMER%'
            ...
            try
            {
                using (MySqlCommand cmd = new MySqlCommand(query, mySqlConn))
                {
                    ...
                    using (MySqlDataReader reader = cmd.ExecuteReader())
                    {
                        while (reader.Read())
                        {
                            // Puedes crear un objeto o simplemente guardar el id
                            fpList.Add(reader["id_flete"].ToString());
                        }
                    }
            ...
            {
                Toast.MakeText(_activity, "Error al obtener fletes: " + ex.Message, ToastLength.Long).Show();
            }
            finally
            {
        ...
        private async void getFletesPendientesPHP()
        {
            fpList.Clear();

            try
            {
                string url = "http://189.206.160.206:81/EmbarquesApk/api/getFletesPendientes.php";
                if (_activity.usuario == "IVAN")
                {
                    url = "http://189.206.160.206:81/EmbarquesApk/api/getFletesPendientesAGUILARES.php"; // 🔹 cambia esto
                }
                if (_activity.usuario == "COVEMEX")
                {
                    url = "http://189.206.160.206:81/EmbarquesApk/api/getFletesPendientesCOVEMEX.php"; // 🔹 cambia esto
                }
                ...
                using (var client = new HttpClient())
                {
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        {
                            fpList.Add(f.id_flete + " - " + f.orde + "|" + f.id_proveedor + "," + f.id_ranchoDET + "," + f.id_tablaDET);
                        }
        ...
        private void GvFletesPendientes_ItemClick1(object sender, AdapterView.ItemClickEventArgs e)
        {
            try
            {
                string item = fpList[e.Position];
                var datos = item.Split(',');
                string id_flete = datos[0];
                string nom_proveedor = datos[1];
                string origen = datos[2];
                string destino = datos[3];

                // Mostrar info del flete
                MainActivity.ShowDialog("Flete seleccionado",
                    $"ID: {id_flete}\nProveedor: {nom_proveedor}\nDestino: {destino}");

                // Guarda el proveedor actual
                prov_nombre = nom_proveedor.Trim();
                prov_clave = getProv_Clave(prov_nombre);

                // Actualiza el spinner Proveedor automáticamente
                Spinner spinnerProveedor = vwEntradas.FindViewById<Spinner>(Resource.Id.sprProveedor);
                if (spinnerProveedor != null && spinnerProveedor.Adapter != null)
                {
                    for (int i = 0; i < spinnerProveedor.Adapter.Count; i++)
                    {
                        string itemText = spinnerProveedor.GetItemAtPosition(i)?.ToString()?.Trim();
                        if (itemText == prov_nombre)
                        {
                ...
                // Carga ranchos asociados al proveedor
                loadRanchosPorProveedor(prov_clave);

            }
            catch (Exception ex)
            {
                Toast.MakeText(Context, "Error al seleccionar flete: " + ex.Message, ToastLength.Long).Show();
            }
        }
        ...
        private void GvFletesPendientes_ItemClick(object sender, AdapterView.ItemClickEventArgs e)
        {
            ...
            {
                string item = fpList[e.Position];
                // Ejemplo: "123 - COMERCIAL|PROV001,RANCHO01,TABLA05"
                ...
                string idProveedor = datos[0];
                string idRancho = datos[1];
                string idTabla = datos[2];

                prov_clave = idProveedor;
                rch_clave = idRancho;
                tbl_clave = idTabla;

                // 🔹 Actualiza los spinners según el flete
                ActualizarSpinnersDesdeFlete(idProveedor, idRancho, idTabla);

                Toast.MakeText(_activity, $"Flete seleccionado:\nProveedor: {idProveedor}\nRancho: {idRancho}\nTabla: {idTabla}", ToastLength.Short).Show();
            }
            catch (Exception ex)
            {
                Android.Util.Log.Error("GvFletes_ItemClick", ex.Message);
                Toast.MakeText(_activity, "Error al seleccionar el flete: " + ex.Message, ToastLength.Long).Show();
            }
        }

        private void ActualizarSpinnersDesdeFlete(string provClave, string ranchoClave, string tablaClave)
        {
            try
            {
                ...
                if (vwProveedor != null)
                {
                    var provRow = vwProveedor.Select($"prov_clave = '{provClave}'");
                    if (provRow.Length > 0)
                    {
                        string provNombre = provRow[0]["prov_nombre"].ToString().Trim();
                        int index = ((ArrayAdapter)spinnerProv.Adapter).GetPosition(provNombre);
                        if (index >= 0)
                            spinnerProv.SetSelection(index);
                    }
                }

                // 🔹 Cargar ranchos del proveedor
                loadRanchosPorProveedor(provClave);

                System.Threading.Tasks.Task.Delay(300).Wait(); // espera breve

                // 🔹 Seleccionar rancho
                if (vwRanchos != null)
                {
                    var ranchoRow = vwRanchos.Select($"rch_clave = '{ranchoClave}'");
                    if (ranchoRow.Length > 0)
                    {
                        string ranchoNombre = ranchoRow[0]["rch_nombre"].ToString().Trim();
                        int index = ((ArrayAdapter)spinnerRancho.Adapter).GetPosition(ranchoNombre);
                        if (index >= 0)
                            spinnerRancho.SetSelection(index);
                    }
                }

                // 🔹 Cargar tablas del rancho
                loadTablasPorRancho(ranchoClave, provClave);
                ...
                if (vwTablas != null)
                {
                    var tablaRow = vwTablas.Select($"tbl_clave = '{tablaClave}'");
                    if (tablaRow.Length > 0)
                    {
                        string tablaNombre = tablaRow[0]["tbl_nombre"].ToString().Trim();
                        int index = ((ArrayAdapter)spinnerTabla.Adapter).GetPosition(tablaNombre);
                        if (index >= 0)
                            spinnerTabla.SetSelection(index);
                    }
                }
            ...
            {
                Android.Util.Log.Error("ActualizarSpinnersDesdeFlete", ex.Message);
                Toast.MakeText(_activity, "Error al actualizar spinners desde flete: " + ex.Message, ToastLength.Long).Show();
            }
        }
        ...
        public int InsertarEntradas(string tipoMovimiento, string usuario, string entProveedor, string entRancho, string entTabla, string entStatus)
        {
            IdConse = -1;
            ...
            using (SqlConnection connection = new SqlConnection(MainActivity.cadenaConexion))
            {
                using (SqlCommand command = new SqlCommand(query, connection))
                {
                    ...
                    command.Parameters.AddWithValue("@Prov_Clave", getProv_Clave(entProveedor) ?? (object)System.DBNull.Value);
                    command.Parameters.AddWithValue("@Ran_Clave", getRch_Clave(entRancho) ?? (object)System.DBNull.Value);
                    command.Parameters.AddWithValue("@Tab_Clave", getTbl_Clave(entTabla) ?? (object)System.DBNull.Value);
            ...
            return IdConse; // ← Aquí devuelves el ID insertado
        }

        public int InsertarEntrada(string tipoMovimiento, string usuario, string entProveedor, string entRancho, string entTabla, string entStatus)
        {
            IdConse = -1;
        ...
        public override void OnPause()
        {
            ...
            try
            {
                ...
                _activity?.MostrarElementosNavegacion();
            }
            catch (Exception e)
            {
        ...
        public async void OnReaderKeyChanged(BaseReader reader, KeyType type, KeyState state, Java.Lang.Object @params)
        {
            ...
            {
                MainActivity.ShowDialog("AVISO", "Debe de dar Inicio a la captura de la entrada y Seleccionar Provedor, Rancho y Tabla!");
            }
...
WHERE prov_clave IN (@nombreProveedor) ORDER BY rch_nombre ASC";
                ...
                {
                    listaRanchos[i] = vwRanchos.Rows[i - 1]["rch_nombre"].ToString();
                }

                var ranchosAdapter = new ArrayAdapter<string>(_activity, Android.Resource.Layout.SimpleSpinnerItem, listaRanchos);

                Spinner spinnerRanchos = vwEntradas.FindViewById<Spinner>(Resource.Id.sprRancho);
...
ORDER BY prov_nombre ASC";
                ...
                Spinner spinnerTablas = vwEntradas.FindViewById<Spinner>(Resource.Id.sprTabla);
        ...
        private void SetButtonClick()
        {
            btnGuardar.Click += async (s, e) =>
            {
                ...
                // Mostrar overlay y desactivar botón
                ...
                {
                    // Ocultar overlay y activar botón
        ...
        public bool OnTouch(View v, MotionEvent e)
        {
            ...
            {
                Toast.MakeText(_activity, "Debe de dar Inicio a la captura de la entrada!", ToastLength.Short).Show();
                return true;
            }
            return false;
        }
